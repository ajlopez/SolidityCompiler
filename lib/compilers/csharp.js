...
function Compiler() {
    ...
    this.compileContract = function (cmd) {
        var decl = 'public class ' + cmd.name() + ' : Contract {';
        var body = cmd.body().compile(this);
    ...
    this.compileProgram = function (program, options) {
        var result = program.compile(this);
        
        var code = [];
        
        code.push('namespace ' + options.namespace + ' {');
        
        if (options.baseNamespace) {
            code.push('    import ' + options.baseNamespace + ';');
            code.push('');
        }
        
        addIndent(code, result, false);
