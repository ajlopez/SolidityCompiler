...
function InterfaceCommand(name, body, is) {
	this.cmdtype = function () { return "InterfaceCommand"; };
...
function Parser(text) {
    ...
    this.interfaces = function () { return interfaces; }
    ...
    parsefns.interface = parseInterfaceCommand;
    ...
    function parseContractCommand() {
        ...
        if (tryParseToken(TokenType.Name, 'is')) {
            var pname = parseContractName();
            is = [ pname ];
            
            while (tryParseToken(TokenType.Punctuation, ','))
                is.push(parseContractName());
        }
    ...
    function parseInterfaceCommand() {
        ...
        if (tryParseToken(TokenType.Name, 'is')) {
            var pname = parseInterfaceName();
            is = [ pname ];
            
            while (tryParseToken(TokenType.Punctuation, ','))
                is.push(parseInterfaceName());
        }
        
        var body = parseCommand();
        
        var cmd = new InterfaceCommand(name, body, is);
    ...
    function parseContractName() {
        ...
        if (!name || !contracts[name])
            throw new Error("Contract name expected");
